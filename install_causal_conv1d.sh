#!/usr/bin/env bash
# 从 GitHub 克隆 causal-conv1d 并按 GPU 架构本地编译（不要装 PyPI wheel）。
#
# Usage:
#   bash install_causal_conv1d.sh h100     # NVIDIA H100, sm_90, tag v1.5.2
#   bash install_causal_conv1d.sh 5090     # GeForce RTX 5090, sm_120, tag v1.5.4
#
# Optional env:
#   CUDA_HOME   default /usr/local/cuda-12.8
#   MAX_JOBS    compile parallelism (default 2)
#   SRC_DIR     existing clone path; if unset, clone into ./third_party/causal-conv1d
set -euo pipefail
...
SRC_DIR="${SRC_DIR:-$ROOT/third_party/causal-conv1d}"
...
echo "[causal-conv1d] GPU=$GPU tag=$GIT_TAG arch=$ARCH_LIST CUDA_HOME=$CUDA_HOME"
