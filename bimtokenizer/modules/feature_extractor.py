...
class MelFeatureExtractor(SequenceFeatureExtractor):
    ...
    def _device_buffers(self, device: torch.device, dtype: torch.dtype = torch.float32):
        """按设备缓存 hann window 与 mel filterbank，避免每步重新分配/搬运。"""
        cache = getattr(self, "_buf_cache", None)
        if cache is None:
            cache = {}
        ...
        if key not in cache:
            ...
            mel_fb = torch.as_tensor(self.mel_filters, device=device, dtype=dtype)
            cache[key] = (window, mel_fb)
        return cache[key]

    def _torch_extract_fbank_features(self, waveform: np.array, device: str = "cpu") -> np.ndarray:
        """
        Compute the log-mel spectrogram using PyTorch STFT with batching,
        yielding results similar to CPU computing with 1e-5 tolerance.
        """
        wav = torch.from_numpy(waveform).to(device, torch.float32)
        window, mel_filters = self._device_buffers(wav.device, wav.dtype)

        if self.dither != 0.0:
            wav = wav + self.dither * torch.randn(wav.shape, dtype=wav.dtype, device=wav.device)

        stft = torch.stft(wav, self.n_fft, self.hop_length, window=window, return_complex=True)
